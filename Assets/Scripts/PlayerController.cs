...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Input.GetMouseButtonDown(0) && jumpCount <2 ) // 마우스 왼쪽 버튼을 눌렀으며 && 최대 점프 횟수가2에 도달하지 않았다면
        {
            jumpCount++; // 점프 횟수 증가
            playerRigidbody.velocity = Vector2.zero; // 점프직전에 속도를 순각적으로 제로로 변경

            playerRigidbody.AddForce(new Vector2(0, jumpForce)); // 리지드바디에 위쪽으로 힘 주기

            playerAudio.Play(); // 오디오 소스 재생
        }
        else if(Input.GetMouseButtonUp(0) && playerRigidbody.velocity.y >0) // 마우스왼쪽 버튼에서 손을 떼는 순간 && 속도의 y값이 양수라면(위로 상승중)
        {
    ...
    private void Die ()
    {
        ...
        playerAudio.clip = deathClip; // 오디오 소스에 할당된 오디오 클릭을 deathClip으로 변경
